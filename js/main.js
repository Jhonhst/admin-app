$(function(){
  ...
  function fechaActual(){
    $.get('http://0.0.0.0:8080/config/fecha.php', '' , function(respuesta) {
  ...
  function enviar(formulario){ 
    ...
    $.post('http://0.0.0.0:8080/config/guardar.php', postData , function(response) {
      alert(response);
      mostrar_datos_deldia();
    });  
  }
 ...
 function mostrar_lista_tiempo(postdata){
  ...
  $.post('http://0.0.0.0:8080/config/traer-tiempo.php', postdata , function(response) {  
  ...
  function mostrar_datos_deldia(){
    ...
    if( fecha != null){//por ciacaso el campo esta en blanco no ejecutes nada
      ...
      $.get('http://0.0.0.0:8080/config/mostrar-alguardar.php', postData , function(response) {
  ...
  $(document).on('click', '.modificar-inversion ', function() {
     ...
     $.post('http://0.0.0.0:8080/config/traer-modificar.php', {id_inversion}, function(response){
  ...
  $(document).on('click', '.modificar-gastos', function() {
     ...
     $.post('http://0.0.0.0:8080/config/traer-modificar.php', {id_gastos}, function(response){
      const dato = JSON.parse(response);
 ...
 function modificar_dia(posData){
  $.post('http://0.0.0.0:8080/config/traer-modificar.php', posData, function(response){
    mostrar_datos_deldia();
    alert(response);
  })
  ...
  $(document).on('click', '.eliminar-inversion', function() {
    if(confirm('¿Desea eliminar esta inversión?')) { 
      let id_inversion = $(this).attr('miId');
      $.post('http://0.0.0.0:8080/config/eliminar.php', {id_inversion}, function (response) {
        alert(response);
        mostrar_datos_deldia();
      });
    }  
  ...
  $(document).on('click', '.eliminar-gasto', function() {
    if(confirm('¿Desea eliminar este gasto?')) { 
      let id_gastos = $(this).attr('miId');
      $.post('http://0.0.0.0:8080/config/eliminar.php', {id_gastos}, function (response) {
        alert(response);
        mostrar_datos_deldia();
      });
    }  
  ...
  $('#modificar-venta').click(function(e){
    ...
    if(isNaN(efectivo) || isNaN(dineroquellevo) || isNaN(dineroextra)){//validación
      ...
      var formulario = $("#venta").serializeArray();
      $.post('http://0.0.0.0:8080/config/traer-modificar.php', formulario, function(response){
        mostrar_datos_deldia();
        alert(response);
      })
      e.preventDefault();
  ...
  $(document).on('click', '.llamar-individual', function() {
      let fecha = $(this).html();
      $.post('http://0.0.0.0:8080/config/llamar-individual.php', {fecha}, function (respuesta) {
